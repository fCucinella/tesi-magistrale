...
class ModelMutationOperatorsUtils():
    ...
    def NS_on_Dense_list(self, lst, mutation_ratio):
        ...
        permutation = utils.GeneralUtils().generate_permutation(input_dim, mutation_ratio)
        shuffled_permutation = utils.GeneralUtils().shuffle(permutation)
    ...
    def NS_on_Conv2D_list(self, lst, mutation_ratio):
        ...
        permutation = utils.GeneralUtils().generate_permutation(num_of_input_channels, mutation_ratio)
        shuffled_permutation = utils.GeneralUtils().shuffle(permutation)
...
class ModelMutationOperators():
    ...
    def NS_mut(model, mutation_ratio, mutated_layer_indices=None):
        utils.ExaminationalUtils().mutation_ratio_range_check(mutation_ratio)
        ...
        for index, layer in enumerate(layers):
            ...
            if not (len(weights) == 0):
                for val in weights:
                    ...
                    if (len(val.shape) != 1) and layers_should_be_mutated[index]:
                        if layer_name == 'Conv2D':
                            val = ModelMutationOperatorsUtils().NS_on_Conv2D_list(val, mutation_ratio) 
                        elif layer_name == 'Dense':
                            val = ModelMutationOperatorsUtils().NS_on_Dense_list(val, mutation_ratio)
                        else:
                            pass
                    new_weights.append(val)
                layer.set_weights(new_weights)
    ...
    def LD_mut(model, mutated_layer_indices=None):
        LD_model = utils.ModelUtils().model_copy(model, 'LD')

        # Randomly select from suitable layers instead of the first one 
        index_of_suitable_layers = ModelMutationOperatorsUtils().LD_model_scan(model)
        ...
        if mutated_layer_indices == None:
            ...
            utils.ExaminationalUtils().in_suitable_indices_check(index_of_suitable_layers, mutated_layer_indices)
    ...
    def LAm_mut(model, mutated_layer_indices=None):
        LAm_model = utils.ModelUtils().model_copy(model, 'LAm')
        copied_LAm_model = utils.ModelUtils().model_copy(model, 'insert')

        # Randomly select from suitable spots instead of the first one 
        index_of_suitable_spots = ModelMutationOperatorsUtils().LAm_model_scan(model)
        ...
        if mutated_layer_indices == None:
            ...
            utils.ExaminationalUtils().in_suitable_indices_check(index_of_suitable_spots, mutated_layer_indices)
    ...
    def AFRm_mut(model, mutated_layer_indices=None):
        AFRm_model = utils.ModelUtils().model_copy(model, 'AFRm')

        # Randomly select from suitable layers instead of the first one 
        index_of_suitable_layers = ModelMutationOperatorsUtils().AFRm_model_scan(model)
        number_of_suitable_layers = len(index_of_suitable_layers)
        if number_of_suitable_layers == 0:
